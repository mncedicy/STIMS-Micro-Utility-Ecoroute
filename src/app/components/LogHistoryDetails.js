...
export default function LogHistoryDetails({ inspectedLogNode, customVehicles, user }) {
    ...
    const getPayloadInputSummary = (log) => {
        let htmlBuffer = '';
        if (log.input_distance) htmlBuffer += `<div><strong>DISTANCE COVERED:</strong> ${log.input_distance} ${log.input_unit || 'km'}</div>`;
        ...
        if (log.origin_iata && log.dest_iata) htmlBuffer += `<div><strong>FLIGHT ROUTE:</strong> ${log.origin_iata} ➔ ${log.dest_iata}</div>`;
    ...
    const handlePrintLogPdf = () => {
        ...
        doc.open();
        doc.write(generatePrintHtml(inspectedLogNode, matchingAssetNode, activePlateLabel, getPayloadInputSummary(inspectedLogNode)));
        doc.close();
        ...
        <div className="bg-slate-950/60 border border-slate-800 p-4 rounded-lg min-h-[300px] flex flex-col justify-between font-mono relative">
            <div className="space-y-3 text-[11px]">
                <div className="border-b border-slate-800 pb-1.5 flex items-center justify-between gap-2">
                    ...
                    <button type="button" onClick={() => setIsExportOpen(true)} className="border border-blue-900 hover:border-blue-500 bg-blue-950/40 text-blue-400 hover:text-white text-[9px] font-bold py-1 px-2.5 rounded transition-all uppercase tracking-wider shrink-0 shadow-sm stims-hover-glow cursor-pointer">🚀 Export</button>
                </div>
                ...
                <div className="space-y-1 text-slate-400">
                    ...
                    {/* Component 1: Split Metrics Grid view */}
                ...
                <div className="bg-slate-950 border border-slate-800 rounded p-2 mt-2">
                    ...
                    <div className="text-[9px] text-slate-400 space-y-1" dangerouslySetInnerHTML={{ __html: getPayloadInputSummary(inspectedLogNode) }} />
                </div>
            </div>

            {isExportOpen && (
                <ExportModal user={user} inspectedLogNode={inspectedLogNode} customVehicles={customVehicles} onGeneratePdf={handlePrintLogPdf} onClose={() => setIsExportOpen(false)} />
            )}
        </div>
    );
}
