...
const MapCoordinatePicker = dynamic(
    () => import('./MapCoordinatePicker'),
    {
        ssr: false,
        loading: () => (
            <div className="h-64 bg-slate-950 border border-slate-800 rounded-lg flex flex-col items-center justify-center text-slate-600 text-[10px] font-mono tracking-widest uppercase">
                <span className="animate-pulse">⚡ INITIALIZING TELEMETRY CANVAS...</span>
            </div>
        )
    }
);
...
export default function TransportFormFields({
    activeTab, distance, setDistance, unit, setUnit, customVehicles, selectedCustomVehicle, setSelectedCustomVehicle,
        ...
        <>
            {activeTab === 'vehicle' && (
                <div className="space-y-3 animate-fade-in">
                    <p className="text-[10px] text-slate-500 uppercase tracking-wider font-bold mb-1">📊 Fleet asset carbon emissions intensity calculator</p>
                    <DistanceField
                    ...
                    />
                    <VehicleFields customVehicles={customVehicles} selectedCustomVehicle={selectedCustomVehicle} setSelectedCustomVehicle={setSelectedCustomVehicle} openDropdownKey={openDropdownKey} setOpenDropdownKey={setOpenDropdownKey} />
                </div>
            )}

            {activeTab === 'shipping' && (
                <div className="space-y-3 animate-fade-in">
                    <p className="text-[10px] text-slate-500 uppercase tracking-wider font-bold mb-1">📊 Cargo weight freight logistics calculator</p>
                    <DistanceField
                        distance={distance} setDistance={setDistance} unit={unit} setUnit={setUnit}
                        setOsrmTotalDuration={setOsrmTotalDuration} setOsrmLegsData={setOsrmLegsData} setOsrmWaypointsData={setOsrmWaypointsData}
                    />
            ...
            {activeTab === 'route' && (
                <div className="space-y-3 animate-fade-in">
                    <h4 className="text-blue-500 text-[10px] font-mono font-black tracking-wider uppercase mb-1">📍 SEQUENTIAL HAVERSINE TRANSIT WAYPOINT ROUTE CHECKER MATRIX</h4>
                    <VehicleFields customVehicles={customVehicles} selectedCustomVehicle={selectedCustomVehicle} setSelectedCustomVehicle={setSelectedCustomVehicle} openDropdownKey={openDropdownKey} setOpenDropdownKey={setOpenDropdownKey} />
                    <MapCoordinatePicker
                        coordinates={routeCoordinates} onCoordinatesChange={setRouteCoordinates}
                        setOsrmTotalDuration={setOsrmTotalDuration} setOsrmLegsData={setOsrmLegsData} setOsrmWaypointsData={setOsrmWaypointsData}
                    />
                </div>
            )}
            ...
            {activeTab === 'tax' && (
                <div className="space-y-3 animate-fade-in font-mono text-xs mt-1">
                    <div className="p-3 bg-blue-950/20 border border-blue-900/40 rounded-lg text-slate-400 text-[11px] leading-relaxed font-sans normal-case">
                        <strong className="text-blue-400 font-mono text-[10px] tracking-wider uppercase block mb-1">📊 SARS Carbon Tax Compliance Ledger</strong>
                    ...
                    <div className="grid grid-cols-2 gap-2 font-mono text-xs">
                        <div>
                            ...
                            <input type="date" value={taxStartDate} onChange={(e) => setTaxStartDate(e.target.value)} max={maxDateBoundary} className="w-full bg-slate-950 border border-slate-800 rounded px-3 py-2 text-slate-200 focus:outline-none focus:border-blue-500 font-mono text-xs" />
                        </div>
                        <div>
                            <label className="block text-slate-400 mb-1 text-[11px] uppercase tracking-wider font-bold">ANALYSIS END DATE</label>
                            <input type="date" value={taxEndDate} onChange={(e) => setTaxEndDate(e.target.value)} max={maxDateBoundary} className="w-full bg-slate-950 border border-slate-800 rounded px-3 py-2 text-slate-200 focus:outline-none focus:border-blue-500 font-mono text-xs" />
                        </div>
                    </div>
                </div>
            )}
