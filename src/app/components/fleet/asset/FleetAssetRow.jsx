...
export function FleetMobileCard({ vehicle, onSelectDelete }) {
    ...
    return (
        <div className="p-3.5 bg-slate-950/70 border border-slate-800/80 rounded-lg space-y-2.5 text-xs relative">
...
export function FleetDesktopRow({ vehicle, onSelectDelete }) {
    const l100km = vehicle.combined_mpg ? (235.215 / parseFloat(vehicle.combined_mpg)).toFixed(1) : null;
    const gPerKm = vehicle.co2_tailpipe_gpm ? Math.round(parseFloat(vehicle.co2_tailpipe_gpm) / 1.60934) : null;
